...
Temperature scaling's usual edge over Platt comes from the setting the modern NN-calibration literature studied (Guo et al. 2017: multiclass, roughly balanced, accuracy-at-argmax as the metric). There it wins on four counts — it preserves the argmax exactly (so calibration can't degrade accuracy), a single `T` captures the dominant overconfidence mode when there's no base-rate skew to correct, one parameter is hard to overfit on a small calibration split, and it's multiclass-native (one `T` over the whole softmax). The US filter has none of those properties: binary, base-rate-uncertain, and consumed by ranking/thresholding rather than argmax accuracy. So temperature's advantages don't apply and its `B = 0` sacrifice is exactly wrong for us.

**When temperature would be the right call in this project:** a future *multiclass* head — the natural candidate is classifying action *type* (strike / demonstration / boycott / …) rather than a binary yes/no — where multiclass-native, accuracy-preserving, single-parameter calibration is the standard choice. It is *not* the right call for the binary heads even under severe imbalance (e.g. the sparse ICA head): rarity tempts toward temperature's one-parameter data efficiency, but extreme imbalance is exactly where the prior-log-odds term is largest and `B` matters most, so dropping the intercept to save a parameter sacrifices the load-bearing knob. For the binary heads, keep the free intercept; reach for temperature only when a head goes multiclass.
