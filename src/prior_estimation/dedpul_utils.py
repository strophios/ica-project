"""
The utils module from the DEDPUL replication code (https://github.com/dimonenka/DEDPUL/blob/master/utils.py).
"""
...
def generate_data(
    ...
    mix_data = np.concatenate(
        (
            sampler(0, 1, int(mix_size * (1 - alpha))),
    ...
    target_mix = np.append(
        np.zeros((int(mix_size * (1 - alpha)),)), np.ones((int(mix_size * alpha),))
    )
...
def estimate_cons_alpha_universal(data_pos, data_mix, dens_pos, dens_mix):
    ...
    cons_alpha = 1 - np.min(
        np.apply_along_axis(
            lambda x: dens_mix(x) / dens_pos(x), min(len(data_mix.shape) - 1, 1), data
        )
    )
...
def estimate_cons_poster_universal(data, dens_pos, dens_mix, cons_alpha):
    poster = np.apply_along_axis(
        lambda x: (dens_mix(x) - dens_pos(x) * (1 - cons_alpha)) / dens_mix(x), -1, data
    )
    poster[poster < 0] = 0
    return poster
...
def estimate_cons_poster(points, dmu, ds, distribution, alpha, cons_alpha=None):
    ...
    cons_poster = np.apply_along_axis(
        lambda x: (pm(x) - p1(x) * (1 - cons_alpha)) / pm(x), -1, points
    )
    return cons_poster
