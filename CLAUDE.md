...
- **Tests**: pytest is configured (`pyproject.toml [tool.pytest.ini_options]`, `pythonpath = ["."]`). Run with `pytest` from the project root. Current coverage: 220 tests passing — invariant tests for `FLPULoss` (`tests/test_flpu_loss.py`); the train/val/test split logic + label-construction + id-uniqueness in `create_classifier_data` (`tests/test_data_splits.py`, including 11 Tier 3 Piece 4 label-construction tests added 2026-05); missing-value handling + headline-with-lead concatenation in `data_from_parquet` (`tests/test_data_loading.py`, 11 tests across two classes — Tier 3 Piece 4, added 2026-05); construction/shape/contract tests for the Tier 2 `ClassificationHead` (`tests/test_heads.py`, 20 tests including 6 covering the `metrics` parameter added in 4c and 3 added in Tier 4 Piece 1 pinning the new explicit-`name` keyword-only contract); `LayerLRModel` (`tests/test_layer_lr_model.py`, 13 tests including a sparse-gradient regression and two loss-tracking regressions added post-Tier-2-review); `ClassifierPreprocessor` (`tests/test_preprocessor.py`, 26 tests including the 14 Tier 3 Piece 1 dual-boundary validation tests added in 2026-05); `RunConfig`/`HeadConfig`/sub-configs (`tests/test_cca_config.py`, 89 tests — Tier 3 Piece 3a's 64 plus 25 added in Tier 4 covering the HeadConfig `/`-rejection (Piece 1) and `ResolvedSteps` + `LRScheduleConfig.with_resolved` + the `resolved` field + JSON round-trip of the nested sub-object (Piece 2)); and integration tests for the assembled stack (`tests/test_assembly.py`, 17 tests including 2 Tier 3 Piece 2 Pattern-2-serialization tests and the Tier 4 Piece 1 `build_endpoint_model` duplicate-head-names assertion test).
...
- **Inherited from Tier 3 closeout, still deferred:** I2 (smoke-test backbone-validation-path coverage gap — captured in `engineering-patterns.md` as the canonical boundary case for the synthetic-stand-ins pattern; addressable by adding a backbone mismatch case to the smoke test); I8 full version (the make-cca-metrics interim factoring is in `src/cca_metrics.py`; full version would push more shared train/eval config through `cca_config.py`).
