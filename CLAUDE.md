...
*Last updated: 2026-05-09 (Tier 3 Piece 3 done: training script, eval script, and smoke test all integrated with `RunConfig` + JSON sidecar pattern. I3/I4/I5 boundary work complete; only Piece 4 — original-scope test coverage — remains in Tier 3.).*
...
- **Tier 3 in progress.** Tier 3 (robustness) is the boundary-enforcement spine for the multi-head future. See `docs/notes/tier3-design.md` for the overall framing (boundary-inventory pattern: validate at every boundary; each catches what others miss) and per-piece design reasoning. Pieces planned: 1 (I3, preprocessor input validation), 2 (I5, Pattern-2 serialization round-trip test), 3 (I4, train/eval config coupling — the most architectural), 4 (original-scope test coverage for label construction and missing-value handling).
  ...
  - **Piece 4 still pending.** Original-scope test coverage for label construction in `create_classifier_data` (cca/immig/descriptor boolean combinations) and missing-value handling (`fill_null("")` path in `data_from_parquet`). Less architectural than the boundary pieces.
- **Tier 4 inherits**: M1 (scratch-file raise placement), M3 (`_default_group_fn` separator), M4 (default head-name collision risk). (M2 retired into Tier 3 Piece 1.)
