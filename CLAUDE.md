...
*Last updated: 2026-07-23. Orientation doc for the project as a whole — current state and contracts. This is the full line-by-line reconciliation for the `cca-doca-retrain` arc (merged via PR #1) and the multi-head assembly that followed; it supersedes the earlier partially-reconciled version. For the live data/artifact map and model-state snapshot, see `docs/notes/project-state-and-data-map.md`; for what's next / deferred, `docs/notes/roadmap.md` (the single live next-steps list). For the US/not-US label pipeline (R), see `r/CLAUDE.md`.*
...
pytest is configured (`pyproject.toml [tool.pytest.ini_options]`, `pythonpath = ["."]`). Run with `uv run pytest` from the project root (no venv activation needed). `hypothesis` (dev dependency) backs property-based tests in the diagnostics suite. **Current coverage: 942 Python tests collected** (measured 2026-07-23 via `uv run pytest --collect-only -q`). The suite now spans the CCA/US spines, the diagnostics module, calibration, the multi-head assembly (`test_assemble_ica`, `test_feature_assembly`, `test_artifact_check_ica`), fusion (`test_fusion_combiner`, `test_fusion_sidecar`, `test_fit_fusion`), the DoCA-retrain data path (`test_cca_doca_data`, `test_embed_corpus`, `test_build_ica_eval_set`, `test_ica_eval`), the relevance head (`test_run_relevance`, `test_relevance_slice_eval`), and preflight checks. The R dateline pipeline has its own testthat suite — `Rscript r/tests/run_tests.R` from the project root, **79 passing assertions as of 2026-06** (see `r/CLAUDE.md`; not re-run this session). Representative Python suites:
...
- **Scripts (current):** `src/run_cca_doca_prior.py` (DEDPUL re-estimate on the DoCA-matched population) → `cca_doca/prior_estimate.json`. **Original path:** `src/prior_estimation/lu_classifier.py` + `src/run_prior_estimate.py`.
...
1. **`src/embed_corpus.py`** — runs the frozen DAPT encoder over a corpus once, caches CLS features to `cca_doca/embed_cache/<suffix>/` (`train250k`, `full`, `ldc_9507`, `us_train_ldc`, etc.). This is the only expensive (GPU/cluster) step.
...
- `escalation.py` — `top_n_group_fn` (per-layer discriminative-LR grouping) + `escalation_decision` (frozen-probe → fine-tune logic).
...
- **`src/model_setup/layer_lr_model.py` — `LayerLRModel`** (a `keras.Model`). Overrides `train_step` to scale gradients by per-variable multipliers before optimizer apply (discriminative fine-tuning + gradual unfreezing). Contracts/gotchas:
  ...
  - **Pre-flight note for encoder-unfreeze work:** `train_step` weights loss by batch size differently than stock Keras (`sample_weight=tf.shape(tf.nest.flatten(x)[0])[0]` vs stock `trainer.py`); resolve before real unfrozen runs (roadmap thread A item 3).
...
**Active thread — the pre-Aug-6 "refine-and-apply" arc (`docs/notes/roadmap.md` is authoritative).** Set by the 2026-07-10 team meeting (`docs/meetings/20260710_notes.md`): refine the model, generate post-1995 ICA candidates, and check recall against DoCA + the team's coded ICA events, with a write-up to the team by **Aug 1** (meeting Aug 6). Committed scope (in order, with an API headline pull running in parallel): US-head diaspora-recall retrain → encoder unfreeze + discriminative LR (payoff attaches to rel and CCA per the own-terms eval) → hyperparameter tuning on the cluster → apply + evaluate → write-up. VAT/ALUM and temporal (time-token) signal are pre-registered stretch items with promotion rules. Do not rewrite the roadmap's runbook detail here — it is the live list.

**Known ceilings / open items:**
- **US-head diaspora recall** — the system-recall ceiling (drops ~27% of ICA positives at τ_us=0.3); scoped retrain in `docs/notes/us-head-retrain-plan.md`.
- **Frozen encoder** — every head sits on a frozen encoder; unfreezing the top layers is the clearest quality lever (rel especially).
...
- **Cluster work** — finish the API 1976–1995 embed and optional stripped-LDC re-embed; `docs/notes/ica-apply-results-and-cluster-runbook.md`.
...
- `ml_memo/ica_model_state_2026-06.md` — collaborator-facing model numbers (composed + decomposed + own-terms).
...
- `docs/notes/us-head-retrain-plan.md` — the deferred US-head retrain design.
...
- **Model-state snapshot for collaborators**: `ml_memo/ica_model_state_2026-06.md` — the multi-head classifier's current numbers and known ceilings
